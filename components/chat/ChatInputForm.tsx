...
export default function ChatInputForm({ numero, socket }: Props) {
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (archivo) {
      ...
      startTransition(() => {
        sendImageAction({ errors: [], success: "" }, formData).then((res: SendImageActionResponse) => {
          if (res.errors?.length) {
            ...
            // Emitimos por socket el mensaje con la URL
            socket.emit("enviar-mensaje", {
              numero,
              mensaje: mensaje || null,
              emisor: "admin",
              tipo: "image",
              mediaUrl: res.success, // ✅ usamos el string directamente
